def curJob = mavenJob('CCQ_BUILD_MERGED') {
    description('Job to build CCQ')
    ...
    parameters {
        stringParam('branch', 'feature/COMPL-760', 'Branch name to build and deploy')
    }
    ...
    scm {
        git {
            ...
            remote {
                url('https://build_agent@bitbucket.wada-ama.org/scm/ccsvy/code-compliance-survey.git')
...
''', Behavior.DoNothing)
        ...
        downstreamParameterized {
            trigger('CREATE_JIRA_TICKET_CCQ') {
                condition('SUCCESS')
                parameters {
